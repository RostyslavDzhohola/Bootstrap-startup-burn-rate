...
import { resetClock } from "@/app/actions";
...
export default function ScenarioPage({
  ...
  // Reset clock handler - wipes out clock data and resets to zero
  const handleReset = async () => {
    if (!isSignedIn) {
      toast.error("Please sign in to reset your clock");
      return;
    }

    if (
      !confirm(
        "Are you sure you want to reset your clock? This will clear all data and cannot be undone."
      )
    ) {
      return;
    }
    ...
    setIsResetting(true);
    try {
      await resetClock(scenario.id);
      toast.success("Clock reset successfully!");
      router.refresh(); // Refresh to show updated state
    } catch (error) {
      toast.error("Failed to reset clock. Please try again.");
        ...
        <div className="text-center space-y-6 w-full">
          ...
          <div className="mt-6 text-center flex gap-4 justify-center items-center">
            ...
            <button
              ...
              className="px-6 py-3 bg-linear-to-br from-slate-900 via-red-950 to-slate-900 text-white rounded-lg font-medium transition-all duration-300 border-2 border-red-500/50 shadow-[0_8px_24px_rgba(0,0,0,0.4),0_0_20px_rgba(239,68,68,0.25),inset_0_1px_0_rgba(255,255,255,0.1)] hover:shadow-[0_12px_32px_rgba(0,0,0,0.5),0_0_30px_rgba(239,68,68,0.4),inset_0_1px_0_rgba(255,255,255,0.1)] disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
            >
              {isResetting ? "Resetting..." : "Reset Clock"}
            </button>
          </div>
